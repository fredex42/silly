...
*/
char lookup_scancode(char scancode, uint8_t shiftmask)
{
    ...
    size_t keymap_pos = (scancode * 2); //code 1 is at position 3+4 (0-based byte 2 + 3), there is no code 0
    if(shiftmask & SHIFTMASK_SHIFT) keymap_pos+=1;  //shifted equivalent is at +1
    if(keymap_pos>keymap_len) {
        kprintf("ERROR scancode 0x%x out of range, limit was 0x%x\r\n", (uint32_t)scancode, (uint32_t)keymap_len);
        return 0;
    }
