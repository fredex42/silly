...
void setup_paging() {
  ...
  */
  ...
  // /*
  // now do identity-paging the first 4 pages
  // */
  // idpaging((uint32_t *)first_pagedir_entry, 0x0, 0x4000);
  // /*
  // identity-page the kernel space (pages 7-18)
  // */
  // idpaging((uint32_t *)first_pagedir_entry, 0x000, 0x19000);
  // /*
  // identity-page the upper memory regions (pages 80-FF)
  // */
  // idpaging((uint32_t *)first_pagedir_entry, 0x80000, 0x100000);
...
void * k_map_page(uint32_t *root_page_dir, void * phys_addr, uint16_t pagedir_idx, uint16_t pageent_idx, uint32_t flags)
{
  ...
  return (void *)(pagedir_idx << 22 | pageent_idx << 12);
}
...
void k_unmap_page(uint32_t *root_page_dir, uint16_t pagedir_idx, uint16_t pageent_idx)
{
  ...
  vaddr vptr_to_invalidate = pagedir_idx << 22 | pageent_idx << 12;
  kprintf("DEBUG k_unmap_page vptr to invalidate is 0x%x\r\n", vptr_to_invalidate);
  __invalidate_vptr(vptr_to_invalidate);
}
...
void* k_map_if_required(uint32_t *base_directory_vptr, void *phys_addr, uint32_t flags)
{
  ...
  if(existing_mapping) {
    //kprintf("DEBUG k_map_if_required existing mapping found for 0x%x at 0x%x\r\n", page_value, existing_mapping);
