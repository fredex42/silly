...
*/
void legacy_pic_remap(uint8_t offset1, uint8_t offset2)
{
  kputs("    Remapping legacy PIC...");
	unsigned char a1, a2;

	inb((uint32_t)PIC1_DATA, a1);                        // save masks
	inb((uint32_t)PIC2_DATA, a2);

	outb(PIC1_COMMAND, (uint8_t)ICW1_INIT | (uint8_t)ICW1_ICW4);  // starts the initialization sequence (in cascade mode)
	io_wait();
	outb(PIC2_COMMAND, (uint8_t)ICW1_INIT | (uint8_t)ICW1_ICW4);
	io_wait();
...
void setup_pic()
{
  ...
  //remap the PIC to send interrupts from vector 0x20
  legacy_pic_remap(0x20, 0x28);
  //set up irq handlers from vector 0x20
  configure_pic_interrupts(0x20);
}
