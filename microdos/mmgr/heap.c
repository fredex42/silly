...
{
  struct HeapZoneStart **kernel_heap_ptr =(struct HeapZoneStart **) KERNEL_HEAP_PTR;
  ...
  void *slab = vm_alloc_pages(NULL, initial_pages, flags);
  ...
  *kernel_heap_ptr = zone;
  //process->heap_allocated = zone->zone_length;

  kputs("initialise_heap completed\r\n");
  return zone;
}
...
{
  struct HeapZoneStart *kernel_heap = * ((struct HeapZoneStart **) KERNEL_HEAP_PTR);
  struct HeapZoneStart* zone = _zone_for_ptr(kernel_heap, ptr);
  if(!zone) {
